...
class AboutUs extends Component {
  ...
  getAboutUsData = () => {
    ...
    FetchPostWithHeader(
      ...
      //console.log('about data : ', JSON.stringify(result));
      if (result.status == 400) {
        Toast.show(result.message);
      } else if (result.status == 200) {
        //Toast.show(result.message);
        this.setState({aboutUs: result.data});
      }
    });
  };
  leftProfileOnPress = () => {
    ...
    return (
      <>
        ...
        <ScrollView style={styles.container}>
          ...
          <View style={styles.headingView}>
            <Text style={styles.descriptionText}>{this.state.aboutUs}</Text>
          </View>
        </ScrollView>
...
export default connect(mapStateToProps, mapsDespathToProps)(AboutUs);
