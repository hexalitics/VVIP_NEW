...
class VenuesInvite extends Component {
  ...
  componentDidMount() {
    //console.log('Today / future list ', this.props.user.isTodayFuture);
    this._unsubscribe = this.props.navigation.addListener('focus', async () => {
      console.log('today future list', this.props.user.isTodayFuture);
      if (this.props.user.isTodayFuture === true) {
        this.setState(
          {
      ...
      } else if (this.props.user.isTodayFuture === false) {
        this.setState(
          {
            venueDetails: this.props.route.params.venueDetails,
            isTodayFuture: 1,
    ...
    FetchPostWithHeader(
      ...
      if (result.status == 400) {
        ...
        this.setState({
          ...
          totalContact: result.totalTodayContact,
          totalChecked: result.totalChecked,
          totalFutureContact: result.totalFutureContact,
        });
        this.props.saveAuthReducersData('isTodayFuture', true);
      }
    });
  ...
  getFutureContact = async () => {
    ...
    FetchPostWithHeader(
      ...
      if (result.status == 400) {
        ...
        this.props.saveAuthReducersData('isTodayFuture', false);
      }
    });
    //console.log(JSON.stringify(this.state.contactList));
  };
              ...
              <View style={{marginVertical: 10, marginHorizontal: 15}}>
                ...
                <View>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      this.setState({includeGuests: !this.state.includeGuests});
                    }}>
                    <CheckBox
                      disabled={false}
                      style={{height: 25, width: 25, paddingRight: 25}}
                      boxType={'square'}
                      tintColor={'#5D5E5D'}
                      onCheckColor={'#5D5E5D'}
                      onTintColor={'#5D5E5D'}
...
const styles = StyleSheet.create({
  ...
  vanueStatusText: {marginBottom: 10, fontSize: 20, fontWeight: 'bold'},
  inviteList: {
