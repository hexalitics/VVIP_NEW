...
class EditInvite extends Component {
  ...
  componentDidMount() {
    this._unsubscribe = this.props.navigation.addListener('focus', async () => {
      ...
      //console.log('venue details123', venueDetails.VVIPGuests);
      this.setState({groupName: venueDetails.VVIPGuests.toString()});
      const lname =
        selectedContactForInviteScreen.lname === undefined
          ? ''
          : selectedContactForInviteScreen.lname.charAt(0);
      let nameAbb = selectedContactForInviteScreen.fname.charAt(0) + lname;
  ...
  inviteUsers = () => {
    ...
    console.log(this.state.selectedContactForInviteScreen.countyCode);
    let postData = {
      venueId: this.state.venueData.venueId,
      //invitedId: this.state.venueData.invittedVVIPId,
      invitedId: this.state.venueData.id,
      VVIPGuests: this.state.venueData.VVIPGuests,
      invitedVVIPphoneNumber: this.state.selectedContactForInviteScreen.number,
      invitedVVIPcountyCode: this.state.venueData.countyCode,
      ...
      invitedVVIPName: this.state.groupName,
      message: '',
      VVIPGuests: this.state.groupName,
    };
    console.log('invite update post data ', JSON.stringify(postData));
    //return false;
    this.setState({isVisibleLoader: true});
    FetchPostWithHeader(
      'api/venueInvitationUpdate',
      postData,
      ...
      <>
        ...
        <ScrollView style={[styles.container]}>
          <View
            ...
            {this.state.selectedContactForInviteScreen.hasThumbnail ? (
              <Image
                style={{
                  padding: 5,
                  borderRadius: 55,
                ...
                source={{
                  uri: this.state.selectedContactForInviteScreen.thumbnailPath,
                }}
              />
            ) : (
              <View
            ...
            <View style={{justifyContent: 'center', alignItems: 'center'}}>
              <Text
                ...
                {this.state.selectedContactForInviteScreen.fullName}
              </Text>
              <Text style={{fontSize: 18, fontWeight: 'bold', margin: 5}}>
                {this.state.selectedContactForInviteScreen.number}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.groupNameTextView}>
            <View
              ...
              <TextInput
                ...
                style={[styles.groupNameText, {flex: 1, height: 45}]}
          ...
          <View style={{marginVertical: 10, alignSelf: 'center'}}>
            <TouchableOpacity
              ...
              <Text style={styles.inviteButton}>Update</Text>
            </TouchableOpacity>
          </View>
          <DateTimePickerModal
...
export default connect(mapStateToProps, mapsDespathToProps)(EditInvite);
